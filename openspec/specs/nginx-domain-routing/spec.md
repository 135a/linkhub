...
## Purpose
定义 Nginx 域名路由与 API 反向代理行为，保障前端入口可访问与后端语义透传。
...
#### Requirement: 后端异常时前端可感知错误
- **WHEN** 后端服务不可用或返回 5xx
- **THEN** 代理层 MUST 返回可识别的错误状态并记录错误日志用于排查

### Requirement: Nginx 必须转发根路径短链接请求至后端
Nginx MUST 能够识别并转发根路径下的短链接访问请求（通常为 4~8 位 Base62 字符）到后端跳转接口，确保跳转逻辑被正确执行。

#### Scenario: 短链接请求正常转发
- **WHEN** 用户访问 `domain.com/xxxxxx`（其中 xxxxxx 为 4~8 位字母数字组合）
