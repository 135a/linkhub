...
本项目基于「拿个 offer 马丁 - SaaS 短链接」进行二次开发与工程化增强，主要在以下方向做了扩展：

- Go 网关限流算法增强（令牌桶 / 固定窗口 / 滑动窗口）
- Java 主链路深挖（缓存一致性、事务消息、JVM 调优、锁机制）
- Go 日志系统可靠性增强（高水位保护 + WAL 恢复）
- 完整压测与可复现实验文档（便于开源复现与面试说明）
...
- 内存：16 GB RAM（可用约 15.2 GB）
- 系统类型：64 位操作系统，x64 处理器

> 注：README 中仅保留与性能复现相关的硬件信息；设备 ID、产品 ID 等敏感标识不建议公开。
