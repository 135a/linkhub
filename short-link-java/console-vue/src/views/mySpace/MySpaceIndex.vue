        ...
        <li v-for="(item, index) in editableTabs" :key="item.gid">
          <div class="item-box flex-box hover-box" :class="{ selectedItem: selectedIndex === index }"
            ...
            <div class="flex-box">
              ...
              <el-dropdown>
                <div class="block">
                  <el-icon class="edit" v-if="item.name !== '默认分组'">
                    <Tools />
                  </el-icon>
                </div>
                <template #dropdown>
            ...
            <template #default="scope">
              <div class="table-url-box">
                ...
                <el-link type="primary" :underline="false" target="_blank"
                  ...
                  :href="buildShortLinkDisplayUrl(scope.row)">{{ scope.row.domain + '/' + scope.row.shortUri }}</el-link>
          ...
          <el-table-column prop="copy" width="170">
            <template #default="scope">
              <div style="display: flex; align-items: center">
                <!-- 二维码 -->
                <QRCode :url="buildShortLinkDisplayUrl(scope.row)"></QRCode>
                <!-- 表格中的复制链接按钮 -->
                <el-tooltip show-after="500" class="box-item" effect="dark" content="复制链接" placement="bottom-end">
                  <el-icon @click="copyUrl(buildShortLinkDisplayUrl(scope.row))" class="table-edit copy-url">
                    <Share />
                  </el-icon>
                </el-tooltip>
              </div>
...
import { buildShortLinkDisplayUrl } from '@/utils/shortLinkPublicUrl.js'
...
const getGroupInfo = async (fn) => {
  ...
  if (res?.data?.success) {
    ...
    ElMessage.error(res?.data?.message || '获取分组列表失败')
  }
}
