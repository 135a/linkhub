...
public class ShortLinkController {
     ...
     * 创建短链接
     */
    @RateLimit(resource = "create_short-link", qps = 300, controlBehavior = RuleConstant.CONTROL_BEHAVIOR_RATE_LIMITER, maxQueueingTimeMs = 2000, message = "创建请求过于频繁，请稍后再试")
     ...
     * 创建短链接（分布式锁版本）
     */
    @RateLimit(resource = "create-by-lock_short-link", qps = 3000, controlBehavior = RuleConstant.CONTROL_BEHAVIOR_RATE_LIMITER, maxQueueingTimeMs = 2000, message = "创建请求过于频繁，请稍后再试")
     ...
     * 批量创建短链接
     */
    @RateLimit(resource = "batch-create_short-link", qps = 5000, controlBehavior = RuleConstant.CONTROL_BEHAVIOR_RATE_LIMITER, maxQueueingTimeMs = 5000, message = "批量创建请求过于频繁，请稍后再试")
    @SneakyThrows
    ...
    public Result<Page<ShortLinkPageRespDTO>> pageShortLink(ShortLinkPageReqDTO requestParam) {
        Result<Page<ShortLinkPageRespDTO>> result = Results
                .success((Page<ShortLinkPageRespDTO>) shortLinkService.pageShortLink(requestParam));
        return result;
    }
